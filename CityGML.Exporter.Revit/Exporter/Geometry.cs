...

    {
        ...
        internal void AddMesh(PolymeshTopology node, Transform currentTransform)
        {
            ...
            foreach (XYZ xyz in points)
            {
                ...
                float x = (float)UnitUtils.ConvertFromInternalUnits(p.X, UnitTypeId.Meters);
                float y = (float)UnitUtils.ConvertFromInternalUnits(p.Y, UnitTypeId.Meters);
                float z = (float)UnitUtils.ConvertFromInternalUnits(p.Z, UnitTypeId.Meters);
#else
