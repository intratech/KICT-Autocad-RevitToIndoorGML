...
namespace IndoorGML.Core
{
	...
	public class CellSpaceMember
	{
		...
		//public GeneralSpace CellSpace { get; set; }
	}
	...
	public class IndoorFeatures
	{
        ...
        {
			var listSpaces = PrimalSpaceFeatures.PrimalSpaceFeatures.CellSpaceMember.Where(x => x.GeneralSpace!= null&& x.GeneralSpace.Class == className && x.GeneralSpace.Usage == usage && x.GeneralSpace.Function == function).ToList();
			...
			listSpaces.ForEach(x =>
			{
				if (!string.IsNullOrEmpty(x.GeneralSpace.Duality.Href))
				{
					...
					var state = nodes.Where(y => y.State.Id == duality).FirstOrDefault();
					if (state != null)
						members.Add(state);
				}
			});
			...
			for(int i=0;i<members.Count;i++)
            {
                ...
                {
					var next = members[j];
					if(current.SamePlace(next,tolerance))
                    {
        ...
        {
			return PrimalSpaceFeatures.PrimalSpaceFeatures.CellSpaceMember.Where(x => x.GeneralSpace.Id == id).FirstOrDefault();
        }
