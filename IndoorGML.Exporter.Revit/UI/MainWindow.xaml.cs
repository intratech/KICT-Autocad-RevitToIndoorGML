...
namespace IndoorGML.Exporter.Revit.UI
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        private  List<string> roomtTypes;
        public List<string> RoomTypes
        {
            get
            {
                if (roomtTypes == null)
                {
                    if (unitSpaceFunction == null)
                    {
                        unitSpaceFunction = new UnitSpaceFunctionExcel();
                        unitSpaceFunction.ReadFile();
                    }
                    roomtTypes = unitSpaceFunction.UnitSpaceClasses.Select(x => x.Name).ToList();
                }
                return roomtTypes;
            }
        }

        private List<string> roomFunctions;
        public List<string> RoomFunctions
        {
            get
            {
                if (roomFunctions == null)
                {
                    if (unitSpaceFunction == null)
                    {
                        unitSpaceFunction = new UnitSpaceFunctionExcel();
                    ...
                    roomFunctions = unitSpaceFunction.FunctionCodes.Select(x => x.Name).ToList();
                }
        ...
        public string DefaultRoomType
        {
            get
            {
                if(defaultRoomType == null )
                {
                    ...
                    defaultRoomType = unitSpaceFunction.UnitSpaceClasses.FirstOrDefault()?.Name;
                }
                return defaultRoomType;
            }
            set
            {
                defaultRoomType = value;
                OnPropertyChanged("DefaultRoomType");
            }
        }

        private string defaultRoomFunction;
        public string DefaultRoomFunction
        {
            get
            {
                if (defaultRoomFunction == null)
                {
                    if (unitSpaceFunction == null)
                    {
                        unitSpaceFunction = new UnitSpaceFunctionExcel();
                    ...
                    defaultRoomFunction = unitSpaceFunction.FunctionCodes.FirstOrDefault()?.Name;
                }
                return defaultRoomFunction;
            }
            set
            {
                defaultRoomFunction = value;
                OnPropertyChanged("DefaultRoomType");
            }
        }
        ...
        private BitmapImage filterTextImage;

        public BitmapImage FilterTextImage
        {
            get
            {
                if(filterTextImage == null)
                {
                    filterTextImage = new BitmapImage(new Uri(@"Image/FilterText.png",UriKind.Relative));
                }
                return filterTextImage;
            }
            set
            {
                filterTextImage = value;
            }
        }
        //down-arrow.png
        private BitmapImage downArrow;

        public BitmapImage DownArrow
        {
            get
            {
                if (downArrow == null)
                {
                    downArrow = new BitmapImage(new Uri(@"Image/down-arrow.png", UriKind.Relative));
                }
                return downArrow;
            }
            set
        ...
        private void LoadRooms()
        {
            ...
            foreach(var r in rooms)
            {
                ...
                string type = "";
                string function = "";
                external.GetRoomName( (Element)r,out type,out function);
                bool hasChanged = false;
                if(string.IsNullOrEmpty(type) )
                {
                    if (ConfigStairLabels.Contains(r.Name) || ConfigElevatorLabels.Contains(r.Name) || ConfigEscaplatorLabels.Contains(r.Name))
                    {
                        type = "Vertical";
                    }
                    else
                ...
                {
                    function = DefaultRoomFunction;
                    hasChanged = true;
                }
                

                var room = new RoomScheduleItem
                {
                ...
                {
                    //??
                    external.Add(m_doc, (Element)r, room.Type, room.Function);
                }

                room.PropertyChanged += (obj, e) =>
                {
                    external.Add(m_doc, (Element)r, room.Type, room.Function);
                };
            }
        ...
        private void LoadDoors()
        {
            ...
            foreach (var d in doors)
            {
                ...
                var doorSchedule = new DoorScheduleItem
                {
        ...
        private void btnExport_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                ex.Export(m_doc, _model.OutputFile, levelsToExport);
        ...
        {
            txtFilter.Text = "";
            var source = this.RoomSchedule.GetFilter(Environment.COLUMN_LEVEL);
            listBoxFilter.ItemsSource = source;// this.RoomSchedule.Filters.Where(f => f.ColumnName == Environment.COLUMN_LEVEL).Select(f => new CheckBoxModel(f.Id, f.Text, f.Active));
            tbRoomPopupFilterBtnSelectAll.Text = source.Any(i => i.IsChecked == false) ? "Select all" : "Unselect all";
            myPopup.IsOpen = true;
        }
        private void GridRoom_ColName_Click(object sender, RoutedEventArgs e)
        {
            txtFilter.Text = "";
            var source = this.RoomSchedule.GetFilter(Environment.COLUMN_NAME);
            listBoxFilter.ItemsSource = source;// this.RoomSchedule.Filters.Where(f => f.ColumnName == Environment.COLUMN_LEVEL).Select(f => new CheckBoxModel(f.Id, f.Text, f.Active));
            tbRoomPopupFilterBtnSelectAll.Text = source.Any(i => i.IsChecked == false) ? "Select all" : "Unselect all";
        ...
        {
            txtFilter.Text = "";
            var source = this.RoomSchedule.GetFilter(Environment.COLUMN_NUMBER);
            listBoxFilter.ItemsSource = source;// this.RoomSchedule.Filters.Where(f => f.ColumnName == Environment.COLUMN_LEVEL).Select(f => new CheckBoxModel(f.Id, f.Text, f.Active));
            tbRoomPopupFilterBtnSelectAll.Text = source.Any(i => i.IsChecked == false) ? "Select all" : "Unselect all";
        ...
        private void TextBox_TextChanged(object sender, TextChangedEventArgs e)
        {
            ...
            List<CheckBoxModel> itemSource = new List<CheckBoxModel>();

            foreach (CheckBoxModel item in this.RoomSchedule.GetFilter())
            {
                var filterItem = this.RoomSchedule.GetFilterItem(item.Id);
                if (item.Text.IndexOf(txtBox.Text,  StringComparison.OrdinalIgnoreCase) >=0)
                {
                    if (true)
                    {
                        item.IsChecked = true;
            ...
            listBoxFilter.ItemsSource = itemSource;
            FilterRoomSchedule();
        }
    }
}
