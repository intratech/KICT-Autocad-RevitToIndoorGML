...
namespace IndoorGML.Exporter.Revit.Exporter
{
    public class JsonExporter
    {
        ...
        public List<JRoom> ExportRoomsEx(ExternalData data,IEnumerable<Element> rooms, Document doc, out BoundingBoxXYZ box)
        {
            ...
            foreach (var e in rooms)
            {
                ...
                data.GetRoomName(e, out type, out function);
                j_type = type;
                //room type
        ...
        public List<JDoor> ExportDoorsEx(IEnumerable<Element> doors, List<JRoom> jRooms, List<Phase> phases)
        {
            ...
            {                
                ...
                foreach (var room in jRooms)
                {
                    ...
                    {
                        Log.Info($"Door {fIns.Id.IntegerValue} {sId} position {point.X} {point.Y}");
                        Log.Info($"Door {fIns.Id.IntegerValue} {sId} direction {direction.X} {direction.Y} {direction.Z}");
