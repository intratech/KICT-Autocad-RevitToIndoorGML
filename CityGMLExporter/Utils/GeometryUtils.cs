...
namespace CityGMLExporter.Utils
{
    ...
    {
        public static object CityGMLWriter { get; internal set; }

        //***********************************
        //Function content: Get surfaces of room from line of Room
        //Developer: Donny
        //Last modifier:
        //Modification content:
        //Updated on: 30-Aug-19
        //***********************************
        public static SurfaceMember GetSurface(Point3D startPoint, Point3D endPoint, Point3D moveVec)
        {
        ...
        //***********************************  PART I
        //Function content: Get list points of door in 1 room
        //Developer: Donny
        //Last modifier:
        //Modification content:
        //Updated on: 30-Aug-19
        //***********************************
        public static Dictionary<string, Door> FindDoorsWindowsInRoom(JRoom room, List<JDoor> doors, List<JWindow> windows)
        {
            if (room != null && doors != null)
            {
                double width = CityGML.Core.Config.DefaultWidth;
                double heigh = CityGML.Core.Config.DefaultHeight;
        ...
        //***********************************
        //Function content: Get line of door in the 1 room
        //Developer: Donny
        //Last modifier:
        //Modification content:
        //Updated on: 30-Aug-19
        //***********************************
        public static Dictionary<string, DoorSurfaces> GetDoorSurfacesInRoom(Dictionary<string, Door> doorList)
        {
            if (doorList != null)
            {
                double width = CityGML.Core.Config.DefaultWidth;
        ...
        public static void GetDoorSurfaces(KeyValuePair<string, Door> door, double width, double heigh, ref Dictionary<string, DoorSurfaces> doorSurfaceList)
        {
            ...
            double thickn = CityGML.Core.Config.DefaultThickness;
        ...
        public static bool IsHighestRoom(JRoom room)
        {
            IEnumerable<JRoom> anotherRoomOfSameType = null;
            if (room.is_elevator == true)
            {
                anotherRoomOfSameType = Config.Rooms.Where(r => r.is_elevator == true);
            }
            else if(room.is_escalator == true)
            {
                anotherRoomOfSameType = Config.Rooms.Where(r => r.is_escalator == true);
            }
            else if(room.is_stair == true)
            {
                anotherRoomOfSameType = Config.Rooms.Where(r => r.is_stair == true);
            }
            else
            {
                return false;
            ...
            var currentRoomLevel = Config.Levels.Where(l => l.id == room.level_id).FirstOrDefault();
            ...
            {
                var lv = Config.Levels.Where(l => l.id == another.level_id).FirstOrDefault();
                if (lv == null)
                    continue;
        ...
        public static bool IsLowestRoom(JRoom room)
        {
            IEnumerable<JRoom> anotherRoomOfSameType = null;
            if (room.is_elevator == true)
            {
        ...
        public static void GetExteriorDoorList( List<ExteriorDoor> exteriorDoor, List<JDoor> doors, List<JWindow> windows)
        {
            foreach (var door in doors)
            {
                ExteriorDoor exDoor = new ExteriorDoor(door);
            }

            foreach (var window in windows)
            {
                ExteriorDoor exDoor = new ExteriorDoor(window);
            }
        }
    }
}
